"""
Kansas State Board of Nursing (KSBN) — License Search Scraper
URL: https://ksbn.kansas.gov/

NOTE: Selectors for this board have not yet been confirmed by live inspect.
Run with --headed to inspect the page and update selectors below.
...
Usage:
  python kansas_nursing_scraper_v1.py --search-by "License Number" --query "12345"
  python kansas_nursing_scraper_v1.py --search-by "Last Name" --query "Smith"
...
BASE_URL     = "https://ksbn.kansas.gov/"
WAIT_TIMEOUT = 30000
...
def open_search_page(page):
    ...
    # Try to find and click a verification/search link from homepage
    verify_link = page.locator(
        "a:has-text('License Verification'), a:has-text('Verify a License'), "
        "a:has-text('License Lookup'), a:has-text('Licensee Search'), "
        "a:has-text('Verify'), a:has-text('Look Up')"
    ).first
    if verify_link.count() > 0 and verify_link.is_visible():
        log.info("Clicking verification link: %s", verify_link.inner_text().strip())
        verify_link.click()
        page.wait_for_load_state("networkidle")
    else:
        log.warning("No verification link found on homepage — staying on page.")

    log.info("Current URL: %s", page.url)

    # Print all inputs for selector confirmation
    log.info("=== FIELDS ON PAGE (update selectors below if needed) ===")
...
def fill_search_form(page, search_by: str, query: str) -> bool:
    # TODO: Update these selectors after running with --headed and checking logs
    ...
    if search_by.lower() in ("license number", "license_number"):
        field = page.locator(
            ...
            "input[placeholder*='icense']"
        ).first
        if field.count() == 0:
            log.error("License number field not found. Check logs for field list.")
            return False
        field.fill(query)
        ...
        field = page.locator(
            "input[id*='ast'], input[name*='ast'], input[placeholder*='ast']"
        ).first
        if field.count() == 0:
            log.error("Last name field not found. Check logs for field list.")
            return False
        field.fill(query)
...
def wait_for_results(page) -> bool:
    ...
    for phrase in ["no results", "no records", "not found", "0 result"]:
        if phrase in body:
...
def scrape_detail_page(page, url: str) -> dict:
    log.info("Extracting detail from: %s", url)
...
def run_search(search_by: str, query: str) -> dict:
    output = {
        "state":           "Kansas",
        "board":           "Kansas State Board of Nursing (KSBN)",
        "search_by":       search_by,
        "query":           query,
        "result_count":    0,
        "license_details": [],
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if not fill_search_form(page, search_by, query):
                log.warning("Could not fill form — check field selectors in fill_search_form().")
                return output
            ...
            # Try to find detail links
            detail_links = page.locator("table tr a[href*='detail'], table tr a[href*='profile'], table tr a[href*='verification']").all()

            if detail_links:
                for link in detail_links:
                    href = link.get_attribute("href") or ""
                    if href:
                        full = href if href.startswith("http") else f"https://ksbn.kansas.gov/{href.lstrip('/')}"
                        page.goto(full, timeout=WAIT_TIMEOUT)
                        page.wait_for_load_state("networkidle")
                        detail = scrape_detail_page(page, full)
                        output["license_details"].append(detail)
                        page.go_back()
                        page.wait_for_load_state("networkidle")
            else:
                # No detail links — extract from results page directly
...
def parse_args():
    p = argparse.ArgumentParser(description="Kansas State Board of Nursing — License Search Scraper",
                                epilog=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    p.add_argument("--search-by", default="License Number",
                   help="'License Number' or 'Last Name'")
    p.add_argument("--query",  required=True)
    p.add_argument("--output", default="kansas_nursing_results.json")
    return p.parse_args()
