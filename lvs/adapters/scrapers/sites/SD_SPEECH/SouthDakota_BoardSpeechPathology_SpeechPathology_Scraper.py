#!/usr/bin/env python3
"""
SouthDakota_BoardSpeechPathology_SpeechPathology_Scraper.py
Searches South Dakota Board of Examiners for Speech-Language Pathology roster by name or license number.
"""
...
BASE_URL = "https://www.sdboards.org/healthdept/SpeechPath/verify/"
FILE_PREFIX = "SouthDakota_SpeechPath_roster_"
AGE_LIMIT_DAYS = 7
STATE = "South Dakota"
SOURCE = "South Dakota Board of Examiners for Speech-Language Pathology"
PDF_FOLDER = "pdfs"
...
def parse_sd_speechpath_row(spans: list) -> dict:
    """Parse row spans: Last, First, Level(SLP/SLPA), License#, ExpDate, Discipline"""
    row = {
        "Last Name": "",
        "First Name": "",
        "Level": "",
    ...
    try:
        for page in doc:
            ...
            for g in grouped[data_start:]:
                ...
                if len(spans) >= 2:
                    parsed_row = parse_sd_speechpath_row(spans)
...
def save_json(output: dict, out_dir: Path):
    ...
    out_file = out_dir / f"SouthDakota_SpeechPath_results_{ts}.json"
...
def main():
    parser = argparse.ArgumentParser(description="Search South Dakota speech pathology license verification")
    parser.add_argument("--name", dest="full_name", default="", help="Full name (e.g. 'Allison Koch' or 'Koch, Allison')")
