#!/usr/bin/env python3
"""
SouthDakota_BoardAudiologyHearingAid_Audiologist_Scraper.py
Searches South Dakota Board of Hearing Aid Dispensers and Audiologists roster by name or license number.
"""
...
BASE_URL = "https://www.sdboards.org/healthdept/audiology/verify/"
FILE_PREFIX = "SouthDakota_Audiology_roster_"
AGE_LIMIT_DAYS = 7
STATE = "South Dakota"
SOURCE = "South Dakota Board of Hearing Aid Dispensers and Audiologists"
PDF_FOLDER = "pdfs"
...
def parse_sd_audiology_row(spans: list) -> dict:
    """Parse row spans: Last, First, Level(HAD/AUD), License#, EffDate, ExpDate, Discipline"""
    row = {
        "Last Name": "",
        "First Name": "",
        "Level": "",
    ...
    try:
        for page in doc:
            ...
            for g in grouped[data_start:]:
                ...
                if len(spans) >= 2:
                    parsed_row = parse_sd_audiology_row(spans)
...
def save_json(output: dict, out_dir: Path):
    ...
    out_file = out_dir / f"SouthDakota_Audiology_results_{ts}.json"
...
def main():
    parser = argparse.ArgumentParser(description="Search South Dakota audiology license verification")
    parser.add_argument("--name", dest="full_name", default="", help="Full name (e.g. 'Larry Olson' or 'Olson, Larry')")
