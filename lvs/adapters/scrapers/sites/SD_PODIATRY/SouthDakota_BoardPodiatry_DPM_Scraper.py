#!/usr/bin/env python3
"""
SouthDakota_BoardPodiatry_DPM_Scraper.py
Searches South Dakota Board of Podiatry Examiners roster by name or license number.
"""
...
BASE_URL = "https://www.sdboards.org/healthdept/podiatry/verify/"
FILE_PREFIX = "SouthDakota_Podiatry_roster_"
AGE_LIMIT_DAYS = 7
STATE = "South Dakota"
SOURCE = "South Dakota Board of Podiatry Examiners"
PDF_FOLDER = "pdfs"
...
def parse_sd_podiatry_row(spans: list) -> dict:
    """Parse row spans: Last, First, DPM, LicNum, EffDate, ExpDate, Discipline"""
    # Format varies - sometimes First Name and DPM are separate, sometimes together
    row = {
        "Last Name": "",
        "First Name": "",
        "Level": "",
    ...
    # Determine positions based on number of spans
    if len(spans) == 6:
        # Last, FirstDPM, License, EffDate, ExpDate, Discipline
        first_dpm = (spans[1] or "").split()
        row["First Name"] = first_dpm[0] if len(first_dpm) >= 1 else ""
        row["Level"] = first_dpm[1] if len(first_dpm) >= 2 else ""
        row["License #"] = spans[2] if spans[2] else ""
        row["Effective Date"] = spans[3] if spans[3] else ""
        row["Expiration Date"] = spans[4] if spans[4] else ""
        row["Discipline"] = spans[5] if spans[5] else ""
    elif len(spans) == 7:
        # Last, First, DPM, License, EffDate, ExpDate, Discipline
        row["First Name"] = spans[1] if spans[1] else ""
        row["Level"] = spans[2] if spans[2] else ""
        row["License #"] = spans[3] if spans[3] else ""
        row["Effective Date"] = spans[4] if spans[4] else ""
        row["Expiration Date"] = spans[5] if spans[5] else ""
        row["Discipline"] = spans[6] if spans[6] else ""
    else:
        ...
        for i, span in enumerate(spans[1:], 1):
            if i == 1:
                ...
                row["Effective Date"] = span
            elif i == 5:
                row["Expiration Date"] = span
            elif i == 6:
                row["Discipline"] = span
    ...
    try:
        for page in doc:
            ...
            for g in grouped[data_start:]:
                ...
                if len(spans) >= 2:
                    parsed_row = parse_sd_podiatry_row(spans)
...
def save_json(output: dict, out_dir: Path):
    ...
    out_file = out_dir / f"SouthDakota_Podiatry_results_{ts}.json"
...
def main():
    parser = argparse.ArgumentParser(description="Search South Dakota podiatry license verification")
    parser.add_argument("--name", dest="full_name", default="", help="Full name (e.g. 'Tom Ries' or 'Ries, Tom')")
