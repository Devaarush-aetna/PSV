#!/usr/bin/env python3
"""
SouthDakota_BoardPhysicalTherapy_PhysicalTherapy_Scraper.py
Searches South Dakota Board of Physical Therapy Online Verifications roster by name or license number.
"""
...
BASE_URL = "https://www.sdboards.org/healthdept/physicaltherapy/verify/"
FILE_PREFIX = "SouthDakota_PT_roster_"
AGE_LIMIT_DAYS = 7
STATE = "South Dakota"
SOURCE = "South Dakota Board of Physical Therapy"
PDF_FOLDER = "pdfs"
...
def parse_sd_pt_row(spans: list) -> dict:
    """Parse row spans: Last, First, Level(PT/PTA), License#, EffDate, ExpDate, Discipline"""
    row = {
        "Last Name": "",
        "First Name": "",
        "Level": "",
    ...
    try:
        for page in doc:
            ...
            for g in grouped[data_start:]:
                ...
                if len(spans) >= 2:
                    parsed_row = parse_sd_pt_row(spans)
...
def save_json(output: dict, out_dir: Path):
    ...
    out_file = out_dir / f"SouthDakota_PT_results_{ts}.json"
...
def main():
    parser = argparse.ArgumentParser(description="Search South Dakota physical therapy license verification")
    parser.add_argument("--name", dest="full_name", default="", help="Full name (e.g. 'Toni Lais' or 'Lais, Toni')")
