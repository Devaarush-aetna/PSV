...
def _cached_txts(slug: str) -> list[Path] | None:
    ...
    if not board_dir.exists():
        return None
    files = sorted(board_dir.glob("*_[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]_[0-9][0-9][0-9][0-9].txt"))
    if not files:
        return None
    fresh = []
    for f in files:
        m = re.search(r"_(\d{8}_\d{4})\.txt$", f.name)
        if not m:
            continue
        try:
            age = (datetime.now() - datetime.strptime(m.group(1), "%d%m%Y_%H%M")).days
            if age <= REFRESH_DAYS:
                fresh.append(f)
            else:
                return None
...
def _get_txts(board_label: str) -> list[Path]:
    ...
    date_tag = datetime.now(_EST).strftime("%d%m%Y_%H%M")
...
def main() -> None:
    ...
    downloaded_dates = []
    for p in txt_paths:
        m = re.search(r"_(\d{8}_\d{4})\.txt$", p.name)
        if m:
            downloaded_dates.append(
                datetime.strptime(m.group(1), "%d%m%Y_%H%M").strftime("%Y-%m-%d %H:%M EST")
            )
